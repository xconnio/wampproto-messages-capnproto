...
import capnp
from wampproto.messages import unregister as unregister_message
...
class Unregister(unregister_message.IUnregisterFields):
...
def unregister_to_capnproto(u: unregister_message.Unregister) -> bytes:
    ...
    return helpers.prepend_header(unregister_message.Unregister.TYPE, packed_data, b"")


def capnproto_to_unregister(data: bytes) -> unregister_message.Unregister:
    message_data, _ = helpers.extract_message(data)
    unregister_obj = unregister_capnp.Unregister.from_bytes_packed(message_data)

    return Unregister(unregister_obj)
