...
import capnp
from wampproto.messages.unsubscribe import Unsubscribe, IUnsubscribeFields
...
def unsubscribe_to_capnproto(u: Unsubscribe) -> bytes:
    unsubscribe = unsubscribe_capnp.Unsubscribe.new_message()

    unsubscribe.requestID = u.request_id
    unsubscribe.subscriptionID = u.subscription_id

    packed_data = unsubscribe.to_bytes_packed()

    return helpers.prepend_header(Unsubscribe.TYPE, packed_data, b"")


def capnproto_to_unsubscribe(data: bytes) -> Unsubscribe:
    message_data, _ = helpers.extract_message(data)
    unsubscribe_obj = unsubscribe_capnp.Unsubscribe.from_bytes_packed(message_data)

    return Unsubscribe(UnsubscribeFields(unsubscribe_obj))
